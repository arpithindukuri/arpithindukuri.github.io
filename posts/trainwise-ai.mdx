---
dateCreated: "2024-04-13"
dateUpdated: "2024-04-13"
title: TrainWise AI
description:
  TrainWise AI is a workout tracker app that counts your reps for you.
  The app uses the PoseNet AI/ML model, which takes a video feed of the user, and turns it into a set of points for various joints of the body.
  Using these points we can count the number of reps being performed by a user.
thumbnailPath: /assets/trainwise-ai/thumbnail.png
tags:
  [
    {
      name: "React Native",
      iconPath: "/assets/react-icon.png",
      color: "#51BBD8",
    },
    { name: "MobX", iconPath: "/assets/mobx-icon.png", color: "#EA6618" },
    {
      name: "TypeScript",
      iconPath: "/assets/typescript-icon.png",
      color: "#007ACC",
    },
    { name: "Node.js", iconPath: "/assets/nodejs-icon.png", color: "#539E43" },
    { name: "Next.js", iconPath: "/assets/nextjs-icon.png", color: "#808080" },
    {
      name: "Express",
      iconPath: "/assets/expressjs-icon.png",
      color: "#AAAAAA",
    },
  ]
---

<img src="/assets/trainwise-ai/thumbnail.png" alt="TrainWise AI Thumbnail" />

**Video Demo:** [https://www.youtube.com/watch?v=A2UBi_SAaDg](https://www.youtube.com/watch?v=A2UBi_SAaDg)

TrainWise AI is a workout tracker app which uses Google’s PoseNet AI model to count and track your reps for you.

The app was build with React Native, and uses Express.js backend APIs with Node.js.

The app uses a locally saved PoseNet AI/ML model, which takes in the video feed of the user and turns it into a set of points that represent various joints of the body.

Using these points, and how they change over time, we can count the number of reps being performed by a user.

As technical lead, my responsibility was to:

- Ensure successful integration between frontend and backend by advising respective team leads and assisting in development,
- Achieve alignment between system architecture and business objectives by researching and designing the end-to-end system, and
- Enable team members to focus on development by initializing and maintaining necessary development environments, processes, and documentation, as well as reviewing implemented code and enforcing necessary coding standards.

Opening the app takes us into a home page.

<img
  src="/assets/trainwise-ai/trainwise-ai-1.png"
  alt="TrainWise AI Home Page"
  width="200"
/>

We can see past workouts on the workouts page.

<img
  src="/assets/trainwise-ai/trainwise-ai-2.png"
  alt="TrainWise AI Past Workouts"
  width="200"
/>

The current workout page has workout templates to choose from, or you can start a new workout.

<img
  src="/assets/trainwise-ai/trainwise-ai-3.png"
  alt="TrainWise AI Start Workout"
  width="200"
/>

Here, we can add and edit sets for any given workout.

<img
  src="/assets/trainwise-ai/trainwise-ai-4.png"
  alt="TrainWise AI Current Workout Exercises"
  width="200"
/>

Clicking the camera tracker button on an exercise will open the AI camera tracker.

<img
  src="/assets/trainwise-ai/trainwise-ai-5.png"
  alt="TrainWise AI Current Workout"
  width="200"
/>

After setting up your phone on the floor, you can start performing the exercise, and the reps will automatically be counted for you.

<img
  src="/assets/trainwise-ai/trainwise-ai-6.png"
  alt="TrainWise AI Camera Tracker"
  width="200"
/>
